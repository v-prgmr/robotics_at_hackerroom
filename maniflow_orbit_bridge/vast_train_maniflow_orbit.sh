...
# Vast.ai launcher around the shared ManiFlow training workflow.
# Optional destruction occurs only after training and Hugging Face upload have completed.
...
# Never invoke the provider-specific shutdown block in the shared RunPod launcher.
...
TRAIN_LAUNCHER="${TRAIN_LAUNCHER:-${SCRIPT_DIR}/runpod_train_maniflow_orbit.sh}"
...
echo "Starting Vast.ai Orbit ManiFlow workflow"
...
if is_true "${VAST_DESTROY_ON_EXIT}"; then
    if is_true "${VAST_DESTROY_ON_SUCCESS_ONLY}" && [[ "${TRAIN_EXIT_CODE}" -ne 0 ]]; then
        echo "Not destroying Vast instance because the workflow exited with code ${TRAIN_EXIT_CODE}."
    else
        should_destroy=true
    fi
fi
...
request = urllib.request.Request(
    ...
    headers={
        ...
        "User-Agent": "orbit-vast-launcher/1.0",
    },
    method="DELETE",
)
try:
