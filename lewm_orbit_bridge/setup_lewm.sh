...
mkdir -p "$(dirname "${LEWM_ROOT}")" "$(dirname "${STABLEWM_ROOT}")" "$(dirname "${VENV_DIR}")"
...
uv pip install --python "${VENV_DIR}/bin/python" -e "${STABLEWM_ROOT}[train]" -r "${ORBIT_DIR}/lewm_orbit_bridge/requirements-convert.txt"
